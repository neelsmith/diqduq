...
First, calibrate against your own configured model (from the repo root):
...
- One of *this codebase's own* `UserWarning`s names an actual `max_tokens=<some number>` and the analysis/segmentation still succeeds after retrying -- this is `analyze_with_retry()`/`segment_with_retry()` working as designed. If it's landing on a larger budget than it feels like it should need, `utilities/calibrate_max_tokens.py` (above) is the fix for the analysis stage; segmentation has no calibration script yet (see its own note below) so widening its own fallback constants in `token_budget.py` is the equivalent move there.
- A hard *error* from your provider (rather than a warning) -- something like "tokens exceeded allowed length" as a rejection, not a truncated response -- means the *requested* `max_tokens` itself exceeded what your model actually allows, almost always because `token_budget.py`'s `DEFAULT_CEILING` (`8192`) is bigger than your configured `MODEL`'s real max-output-tokens limit. Neither `utilities/calibrate_max_tokens.py` nor the retry wrappers can fix that by themselves (they only ever ask for *less* than `ceiling`, never more); pass your model's real limit explicitly as `ceiling=`:
...
(`utilities/calibrate_max_tokens.py --calibration-ceiling` is the equivalent knob for the calibration run itself -- lower it if even calibration's own generous default of `8000` gets rejected outright.)
...
- `utilities/` — standalone maintenance scripts, run directly (not imported), each `sys.path`-adjusted to resolve `diqduq_main.py` and `tests/conftest.py` from one directory up (the repo root):
  ...
  - `build_api_docs.py` — regenerates `docs/diqduq-api-docs.html`, a single self-contained HTML page documenting every name in `diqduq.__all__`, built with `pdoc` straight from the package's own docstrings and type hints. Run `python utilities/build_api_docs.py` after changing a public docstring or signature to refresh it; requires `pdoc` (`pip install pdoc --break-system-packages`). Its output stays in `docs/`, not `utilities/`, regardless of where the script itself lives.
- `diqduq/` — the package with the actual analysis logic:
  ...
  - `gepa_metric.py` — `syntax_metric()`, the scoring/feedback function `utilities/optimize_gepa.py` uses with `dspy.GEPA` (see OPTIMIZING.md).
...
- `docs/diqduq-api-docs.html` — the generated API reference page itself (see `utilities/build_api_docs.py` above), plus `docs/index.html`, a small static landing page linking to it.
...
- `notes/` — this file (`USAGE.md`) plus `TESTING.md`, `OPTIMIZING.md`, and `DEVELOPMENT.md`.
