...
Needs the same `.env` as `diqduq_main.py` (`API_BASE`/`MODEL`/`API_KEY`/`MAX_TOKENS`). Optionally set `REFLECTION_MODEL` (and `REFLECTION_API_BASE`/`REFLECTION_API_KEY`/`REFLECTION_MAX_TOKENS`, if they differ) to use a different model specifically for GEPA's reflective step -- GEPA's own docs recommend a strong reasoning model for this. Without `REFLECTION_MODEL` set, the task model doubles as the reflection model, a reasonable default for a first run. `REFLECTION_MAX_TOKENS` falls back to `MAX_TOKENS`, then to the same `4096` default `diqduq_main.py` uses, for the same reason (see USAGE.md's note on `MAX_TOKENS` -- it keeps the reflection LM's own truncation warnings, if any, from misleadingly saying `max_tokens=None`).

**Scope and data**: this optimizes only `SyntaxAnalysis` (the `analyze` module in `hebrew_syntax_dspy.py`), not the segmentation stage. It trains on all gold examples in `GOLD_EXAMPLES` (8 as of this writing) with no separate held-out valset -- per `dspy.GEPA`'s own behavior when no valset is given, it uses the trainset for both reflective updates and Pareto-score tracking. That's a reasonable starting point while the gold set is still small and the scheme itself is a first draft, but expect the optimized prompt to fit these exact sentences well without a guarantee it generalizes to new ones -- worth revisiting (holding out a few examples as a valset) once there are more gold examples to spare.
